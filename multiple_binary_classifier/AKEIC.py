...
# Select images with AKIEC
dfAKIEC = df[df["AKIEC"] == 1][["image", "AKIEC"]]
dfNotAKIEC = df[df["AKIEC"] == 0][["image", "AKIEC"]]

# Balance the dataset by undersampling
dfNotAKIEC = dfNotAKIEC.sample(frac=0.04, random_state=42)
df = pd.concat([dfAKIEC, dfNotAKIEC]).sample(frac=1, random_state=42)
...
report = classification_report(yTest, preds, target_names=["Not AKIEC", "AKIEC"])
print(report)

# Save model
model.save("AKIEC_model.h5")
