...
# Select images with DF
dfDF = df[df["DF"] == 1][["image", "DF"]]
dfNotDF = df[df["DF"] == 0][["image", "DF"]]

# Balance the dataset by undersampling
dfNotDF = dfNotDF.sample(frac=0.04, random_state=42)
df = pd.concat([dfDF, dfNotDF]).sample(frac=1, random_state=42)
...
report = classification_report(yTest, preds, target_names=["Not DF", "DF"])
print(report)

# Save model
model.save("DF_model.h5")
