...
# Select images with BCC
dfBCC = df[df["BCC"] == 1][["image", "BCC"]]
dfNotBCC = df[df["BCC"] == 0][["image", "BCC"]]

# Balance the dataset by undersampling
dfNotBCC = dfNotBCC.sample(frac=0.04, random_state=42)
df = pd.concat([dfBCC, dfNotBCC]).sample(frac=1, random_state=42)
...
report = classification_report(yTest, preds, target_names=["Not BCC", "BCC"])
print(report)

# Save model
model.save("BCC_model.h5")
