...
# Select images with NV
dfNV = df[df["NV"] == 1][["image", "NV"]]
dfNotNV = df[df["NV"] == 0][["image", "NV"]]

# Balance the dataset by undersampling
dfNotNV = dfNotNV.sample(frac=0.04, random_state=42)
df = pd.concat([dfNV, dfNotNV]).sample(frac=1, random_state=42)
